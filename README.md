...
現行ハードウェアの定義です。移行後は8キー仕様に置き換え、外付けRGB LEDとOLEDは使いません。
...
2. 新ハードウェア仕様に合わせて8キーのキーマップモデルを定義する
3. 現行HTMLを参考に、キー定義をTypeScriptデータとして作り直す
4. 現行UIを参考にReactコンポーネントへ分解する
5. WebHID通信モジュールを作る
6. Arduino側に設定用 vendor-defined HID report を追加する
7. ファームウェアを8キー前提のスキャン処理へ変更する
8. 2本の仮想GND `OUTPUT LOW` と8本のDirect `INPUT_PULLUP` を前提に押下状態を読む
9. 外付けRGB LED / OLED 関連処理を削除し、本体LED表示へ整理する
10. SerialプロトコルをHID reportへ置き換える
11. 実機で接続、読み出し、書き込み、再起動後のEEPROM反映を確認する
